...
@section('content')
    <section class="section-1 t-mt-4">
        <div class="t-container t-mx-auto t-px-4">
            ...
            <form action="{{ route('articles.update', $article->id) }}" class="t-grid t-grid-cols-1 t-gap-4 t-mt-4"
                method="POST">
                @method('PUT')
                @csrf
                ...
                <div class="t-flex t-gap-4">
                    ...
                    <a class="t-m-0 btn btn-outline-danger"
                        href="javascript:if (confirm('정말로 삭제하시겠습니까?')) document.article_destroy_form.submit();">삭제</a>
                </div>
            </form>

            <form class="t-hidden" name="article_destroy_form"
